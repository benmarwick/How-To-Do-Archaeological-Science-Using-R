...
#	dat4<-dat3[,c(1, 8, 19, 27, 40, 44, 45, 50, 54, 55, 57, 58, 59, 75)]
	#round(cor(dat4),2) why AB?
	## ----- View/check data
#	head(dat4)
	...
	selected<-rownames(summary(mod.8)$coefficients)[-1]
	
	paramsOut[xx,]<- c(selected, rep(0, 10-length(selected))) ##error here
...
ORs<- exp(cbind(OR = coef(mod.13), confint(mod.13)))[-1,] ## Intercept OR shouldn't be interpreted.
ORs
